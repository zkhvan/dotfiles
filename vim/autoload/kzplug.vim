...
function! kzplug#Exists(name) abort
  return index(get(g:, 'plugs_order', []), a:name) > -1
endfunction

" @param  {String} name
" @return {String} path where plugin installed
function! kzplug#Dir(name) abort
  let l:dir = kzplug#Exists(a:name) ? g:plugs[a:name].dir : ''
...
function! kzplug#IsLoaded(name) abort
  if index(s:loaded, a:name) > -1
    return 1
  endif

  let l:plug_dir = kzplug#Dir(a:name)
