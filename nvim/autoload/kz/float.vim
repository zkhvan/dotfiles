...
" autoload/kz/float.vim

let s:DEFAULT_HEIGHT_RATIO = 0.8
let s:DEFAULT_WIDTH_RATIO = 0.8
function! kz#float#GetOptions(params) abort
  let l:opts = {}
...
function! kz#float#Open(hl, params) abort
  let l:buf = nvim_create_buf(v:false, v:true)
  call setbufvar(buf, '&signcolumn', 'no')
  let l:opts = kz#float#GetOptions(a:params)
...
function! kz#float#Bordered() abort
  let l:opts = kz#float#GetOptions({})
  ...
  " Draw frame
  let g:frame = kz#float#Open('Ignore', {
        \   'row': l:opts.row,
        \   'col': l:opts.col,
        \   'width': l:opts.width,
  ...
  " Draw viewport
  let s:viewport = kz#float#Open('Normal', {
        \   'row': l:opts.row + 1,
        \   'col': l:opts.col + 2,
